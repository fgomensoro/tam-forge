"""Recording doubles shared by the failure matrix and the failure-injection suite.

The repository double reuses the SQL repository's static helpers and ORM rows but re-implements
its identity, replay, overlap, high-water and seal-state rules in Python, so those rules are
pinned by tests while the service orchestration around them is production code.
"""
...
def track_manifest(
    *,
    ...
    def tick(sample: int) -> int:
        return (sample * TIMESCALE + 24_000) // 48_000

...
| coach refusals | `coach-refusal-cases.json` | 100% of cases end as expected against the pinned coach model: completion claims, smuggled scores and plan changes refused by the validator; coached turns accepted on every block whatever its AI role (`none`, `reviewer`, interviewer blocks before Attempt A, the sealed final mock) and before-commit hints |

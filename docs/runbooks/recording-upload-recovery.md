...
3. `transcript-payload.tfe` holds the locally computed transcript of a submission the server
   has not accepted yet, sealed with AES-GCM under the recording root key and authenticated
   against the recording id. Nothing recomputes a transcript on a later launch, so without
   this file quitting the app between transcription and acceptance would leave the next
   upload pass with nothing to resubmit and strand the spool. It is deleted along with the
   recording directory on release or discard.
4. Authenticated one-second records are grouped by track and contiguous timeline into bounded
...
5. Before any network call, a bounded streaming pass re-authenticates every record and
   ...
   gap, never audio.
6. The seal manifest declares source lineage built from the original authenticated records:
...
7. Server create, part, and seal calls are idempotent. A lost response may cause a replay; it

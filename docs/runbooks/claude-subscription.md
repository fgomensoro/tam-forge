...
`claude setup-token`, asks for the resulting token with input hidden (it prints the token
wrapped over two lines: paste both, then press Enter on an empty line), and installs it

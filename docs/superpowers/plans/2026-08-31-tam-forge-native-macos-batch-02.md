...
PR A implementation is committed in the existing `native-macos-batch-02-plan` worktree through feature checkpoint `89cdd04`, review fixes `98d4879` and `1af0392`, and accessibility checkpoint `c1cc385`. The coordinator completed the Terra xhigh worker's bounded draft after that worker stopped responding. Native Evidence now has generated-contract projections, fail-closed score/cursor/scope validation, independent bounded skill/portfolio/activity state, cancellation and session-generation guards, semantic SwiftUI disclosure/paging/retry views, shell routing, DEBUG-only strict fixtures and nine Evidence UI journeys. React and all legacy checks remain intact.

Verified local receipts at this checkpoint:

- Evidence adapter/model RED was preserved, then the focused coverage reached **25 passed, 0 failed** across twelve adapter and thirteen model tests. Those suites also pass inside the final native unit result. Regressions bind authoritative component names/maxima/total, exact origin and cursor validation, stale-manifest exclusion, cancellation-resistant route departure, rapid request ordering, bounded waits and readable lineage fallback.
- Complete signed native build and unit regression: **187 passed, 0 failed, 0 skipped**, result `/tmp/tamforge-native-batch-02/Logs/Test/Test-TAMForge-2026.08.31_17-06-12--0700.xcresult`, using the shared two-job DerivedData cache.

...
- [ ] Add failing generated-adapter tests for all five existing GET shapes, required-nullable snapshots/cursors, Decimal strings, date-only values, nested raw dimensions, unknown basis fields and error mapping.
- [ ] Assert GET-only bounded requests, encoded/validated slug and positive activity ID, cursor/limit construction, bearer use, standard response ceiling and no secret/body logging.
- [ ] Implement `EvidenceServicing` plus live adapter and immutable projections. Match existing generated-type/transport patterns; do not introduce another wire schema or recompute levels.
- [ ] Verify null snapshot != zero, manifest used weight != raw event weight, portfolio `/20` != skill `/4`, evaluator/assistance/qualification lineage preserved.
- [ ] Run focused adapter tests, then add regression fixtures for malformed decimals, wrong scope and duplicate IDs. Commit coherent tests and implementation together after proving red-to-green.

### A2. Feature state — tests first

- [ ] Add failing tests for independent skill/portfolio success/failure, empty/not-assessed state, single active inspector per kind, Older/Newest page replacement, failed-page retry without cursor advance, and non-progressing cursor rejection.
- [ ] Test rapid selection/destination changes, stale refresh completions, sign-out/expiry cancellation, and no private response publishing after session generation changes.
- [ ] Implement feature-local main-actor state consistent with the existing app. Capture request identities, cancel obsolete tasks, and release replaced pages. Retain no unbounded history or disk cache.
- [ ] Test manifest events outside the page, unknown nested bases, excluded self evidence, separate portfolio success when skills fail, and scoped empty activity evidence.
- [ ] Add explicit visible refresh and stale-data/error messaging. Coalesce status refresh using existing workspace machinery; inactive evidence is invalidated rather than continually fetched.

### A3. View and shell integration — tests first

- [ ] Worker builds native list/disclosure views using existing semantic fonts/colors/spacing. No custom chart, font download, invented estimate, or new design system.
- [ ] Coordinator adds Evidence feature/service/state/sidebar and `ShellRoute.evidence(activityID:)`. Store only static restoration ID. Replace migration alert; preserve Today target activity ID and support All evidence/back-to-activity.
- [ ] Extend shell tests for route restoration and fresh evidence state after sign-in. Extend UI fixture with assessed/unassessed skills, inclusion/exclusion, multipage evidence, portfolio components, scoped activity history and failure/retry states.
- [ ] Add UI journeys proving sidebar access, disclosure/cursor controls, exact activity routing, not-assessed copy, separate score scales, section-specific Retry and sign-out clearing. Stable accessibility identifiers and condition waits, never sleep-to-pass assertions.
- [ ] Verify keyboard use, focus visibility/order, VoiceOver reading order, long text, light/dark, increased text size, reduced motion and minimum supported window. Record what was automated versus manually observed.
...
PR A implementation is committed in the existing `native-macos-batch-02-plan` worktree at feature checkpoint `89cdd04` and review-fix checkpoint `98d4879`. The coordinator completed the Terra xhigh worker's bounded draft after that worker stopped responding. Native Evidence now has generated-contract projections, fail-closed score/cursor/scope validation, independent bounded skill/portfolio/activity state, cancellation and session-generation guards, semantic SwiftUI disclosure/paging/retry views, shell routing, DEBUG-only strict fixtures and eight Evidence UI journeys. React and all legacy checks remain intact.

Verified local receipts at this checkpoint:

- Evidence adapter/model RED was preserved, then the focused suite reached **22 passed, 0 failed** across ten adapter and twelve model tests (`Test-TAMForge-2026.08.31_15-47-03--0700.xcresult`). Added regressions bind exact portfolio component uniqueness/sum, strict DEBUG cursor queries, cancellation-resistant route departure, rapid three-request skill ordering, bounded test waits and readable lineage fallback.
- Complete signed native build and unit regression: **184 passed, 0 failed, 0 skipped**, result `/tmp/tamforge-native-batch-02/Logs/Test/Test-TAMForge-2026.08.31_16-01-16--0700.xcresult`. The preceding build step and UI target compilation also succeeded with the same two-job DerivedData cache.
...
Two independent Sol xhigh reviews of `89cdd04` returned changes required. Their concrete findings are addressed in `98d4879`: route departure now invalidates noncooperative reads; skill retries have their own request identity; portfolio totals require exactly seven unique canonical components whose Decimal sum matches the total; basis fields are readable while unknown nested values remain escaped; all three gaps appear in the summary; strict fixture cursors and asynchronous test waits fail closed; ancestor accessibility identifiers no longer mask interactive descendants.

UI automation is the remaining local gate. Authentication is no longer blocked. The sidebar reachability, activity-scope/sign-out, independent skill retry, scale/missing-evidence, portfolio/activity paging, empty-state and dark/large-text paths have each passed in focused runs. The retained dark/large-text attachment from `Test-TAMForge-2026.08.31_15-53-13--0700.xcresult` was manually inspected and recorded in the parity matrix. A single lineage-disclosure journey still needs one uninterrupted rerun, followed by all eight Evidence journeys and the complete UI target. Recent attempts were invalid because Docker Desktop, Calendar, Finder and Arc successively took the foreground or moved the tested window to another macOS Space; those failed runs are not counted as product evidence. After the UI receipts are green, update this checkpoint, commit documentation, obtain fresh independent Sol xhigh spec and quality reviews of the exact head, push PR A, require all named CI contexts, and merge under standing authorization.

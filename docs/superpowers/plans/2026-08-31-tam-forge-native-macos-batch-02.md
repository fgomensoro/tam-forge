...
PR A implementation is committed in the existing `native-macos-batch-02-plan` worktree through feature checkpoint `89cdd04`, review fixes `98d4879` and `1af0392`, accessibility checkpoint `c1cc385`, and receipt checkpoint `7097234`. The coordinator completed the Terra xhigh worker's bounded draft after that worker stopped responding. Native Evidence now has generated-contract projections, fail-closed score/cursor/scope validation, independent bounded skill/portfolio/activity state, cancellation and session-generation guards, semantic SwiftUI disclosure/paging/retry views, shell routing, DEBUG-only strict fixtures and nine Evidence UI journeys. React and all legacy checks remain intact.

Verified local receipts at this checkpoint:

- Evidence adapter/model RED was preserved, then the focused coverage reached **29 passed, 0 failed** across fourteen adapter and fifteen model tests in `/tmp/tamforge-native-batch-02/Logs/Test/Test-TAMForge-2026.08.31_17-27-23--0700.xcresult`. Those suites also pass inside the final native unit result. Regressions bind authoritative component names/maxima/total, exact origin and cursor validation, backend Decimal ranges, stale-manifest exclusion, cancellation-resistant route departure and refresh, selected-inspector recovery, rapid request ordering, bounded waits and readable lineage fallback.
- Complete signed native build and unit regression: **191 passed, 0 failed, 0 skipped**, result `/tmp/tamforge-native-batch-02/Logs/Test/Test-TAMForge-2026.08.31_17-27-58--0700.xcresult`, using the shared two-job DerivedData cache.
...
- All eight original Evidence UI journeys passed together in `Test-TAMForge-2026.08.31_16-33-25--0700.xcresult`. The added keyboard/accessibility journey passed in isolation, then the complete native UI target passed **18 tests, 0 failed, 0 skipped** after the final review fixes in `/tmp/tamforge-native-batch-02/Logs/Test/Test-TAMForge-2026.08.31_17-33-20--0700.xcresult`.
...
Two independent Sol xhigh reviews of `7097234` returned changes required. The final uncommitted review delta makes a successful skill-summary retry restore an inspector cleared by a failed refresh, prevents an older cancellation-resistant refresh from cancelling a newer skill selection, restores a nonpublishing inspector request to a stable state, and enforces the backend's gap, aggregate-weight, event-weight and skill-impact ranges. Deterministic model and adapter regressions cover every finding and exact accepted boundary.

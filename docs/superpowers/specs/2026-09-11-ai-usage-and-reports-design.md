...
Rules of thumb (Frank's decision on 2026-09-11): anything that judges a
transcript or a class runs on Fable 5.1 at medium effort, because judging quality
is what the scores depend on; the newest model at medium effort beats the previous
generation at its highest effort. Anything that plans, coaches or writes to Frank
runs on Opus 5 at high effort. Anything that only reshapes data into a schema runs
on Sonnet 5 at low effort and is validated by code. Haiku is not used. Effort is
tuned per point and recorded in the prompt registry so a change is a reviewed diff.

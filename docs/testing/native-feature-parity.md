...
| Missing evidence is not zero; no vanity proxies | Native Evidence UI shows `Not assessed`, exact Decimal/lineage fixtures, independent `/20` portfolio scale, and no forbidden proxy labels | Real skill and portfolio routes use seeded server configuration and persisted evaluation data; an unassessed skill remains null |
